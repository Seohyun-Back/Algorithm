...
- 왼쪽에서 오른쪽으로 한번 경사로 두기 (→ 방향으로 탐색하면서 더 낮아지는 쪽에만 둠)
    
    → 경사로 둔곳은 rampArr[i] = true로 변경
    
- 오른쪽에서 왼쪽으로 경사로 두기 (← 방향으로 탐색하면서 더 낮아지는 쪽에 경사 두기, rampArr이 true로 이미 경사로 둔 곳이면 겹치므로 false 리턴)
